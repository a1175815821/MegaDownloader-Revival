...
| バージョン | 日付 | テーマ |
| --- | --- | --- |
| 2.5.1 | 2026-09-14 | パッチ版：レジューム維持、速度制限単位修正、監査フォロー2件 |
...
パッチ版。2.5.0 以降の監査フォロー2件——自癒有効化後の回帰＋残りのユーザー可視 issue。一言：**レジュームは維持、速度制限は表示通り**。

### 🐛 第1弾：ウォッチドッグ、AddLinks、7z クォータ、速度制限、隠しリンク
...
- 120 秒の総時間ウォッチドッグ→無進展タイムアウトに変更：チャンクが進むたび計時リセット、大ファイルが無条件失敗しない；自癒とクォータ復帰はチャンク状態と `.part` を保持し、再試行は再開する（再DL循環なし）
- 「オンライン視聴」キャンセルでボタンが固まらない：共有解決継続が両ボタン＋進行中フラグを復位（従来は誤ボタン復位・フラグ残留）
- 保存先不存在時は作成して続行——初回クリックの無反応を解消
- 外部 7z CLI 経路に 50 GiB 展開後サイズ上限を適用（エントリ数上限は既存通り）
- 0.5 MB/s 等の小数制限を保存可能に（Double 解析→KB 四捨五入）；単位表記を MB/s に訂正
- 複数バッチの隠しリンクを改行区切りにし、巨大 blob 化による黙示消失を解消

### 🐛 第2弾：11 件フォロー（第1弾パッチ由来の回帰 2 件を含む）
...
- `.part` サイズ不一致（置換/切詰/満杯後の短縮）をその場再構築＋チャンク初期化して続行——15 分毎の永久失敗を解消
- 再試行の進捗二重計上を解消（開始時に当回カウンタ初期化；一時停止/再開は影響なし。従来は再試行直後に ~100% 跳躍）
- 速度制限 5 箇所を KB×1024 でバイト換算して制御器へ（従来 1 MB/s 指定が ~1 KB/s 相当）
- 一時停止時間は停滞に不算入；クォータ隔離中の零速は 10 秒監視＋5 秒排出で速やかに赤転（従来 ~150 秒放置）
- 複数 `.elc`/`.dlc` をキュー順次取込（ドラッグ＆ドロップとコマンドライン）；コマンドラインは `.elc` 受理（従来は無反応）；複数 `.dlc` の先頭のみ問題を解消
- 失敗（赤）タスクの強制ダウンロードが可能に：全量リセット後、個別起動（恒久失敗は除外のまま；従来は無反応）
- パッケージプロパティ保存で各ファイル制限が黙示零化されない（均一時は共有値表示、未選択時は上書き抑止）
- 7z 展開は総量を公表し、進捗は 済/共 表示に（バイト粒度は引続きマネージド経路のみ）
- 速度欄は小数4桁まで（`1.46484375`→`1.4648`、往復誤差 <1KB）

### 📦 バージョン番号

- Assembly / FileVersion → `2.5.1.0`；`docs/version.xml` → `2.5.1.0`（2.5.0 以前は本更新の通知対象）
- InternalConfig `VERSION_MEGADOWNLOADER` / `VERSION_UPDATE` → `2.5.1`（`VERSION_UPDATE` は既知 `Double` 解析制限のため統計 ping は 2.5.0 同様の扱い；更新検査自体は `System.Version` 比較で影響なし）
